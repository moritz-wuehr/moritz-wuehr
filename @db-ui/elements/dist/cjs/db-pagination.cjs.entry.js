...
const DbPagination = class {
  constructor(hostRef) {
    ...
    this.ariaid = 'pagination-' + utils.uuid();
  }
  handleChange(event) {
  ...
  decorateChild(child) {
    ...
    return index.h("li", { innerHTML: child.outerHTML });
  }
  paginationButtonFragment({ page, curr, ariaCurrent, ariaDisabled, title, rel, tabindex, disabled, index: index$1 }) {
    return (index.h("li", { key: index$1 }, index.h("button", Object.assign({ class: "elm-link", "data-rel": rel === 'none' ? false : `${rel}`, tabindex: tabindex ? '-1' : false, title: title + ` ${page ? page : ''}`, type: "button" }, ((page === curr || disabled) && ariaCurrent), ((page === curr || disabled) && ariaDisabled), { onClick: (event) => this.handleChange(event) }), page)));
  }
  navElement(children) {
    return (index.h("nav", { class: "cmp-pagination", role: "navigation", "aria-labelledby": this.ariaid }, index.h("h2", { class: "elm-headline ", id: this.ariaid }, this.arialabel.charAt(0).toUpperCase() + this.arialabel.slice(1)), index.h("ol", null, children)));
  }
  slotChildren() {
    ...
    return this.navElement(index.h(index.Fragment, null, this.decorateChild(prevPage), this.decorateChild(firstChild), this.ellipsisstart && index.h("li", null, "\u2026"), tiplessChildren.map(this.decorateChild), this.ellipsisend && index.h("li", null, "\u2026"), this.decorateChild(lastChild), this.decorateChild(nextPage)));
  }
  goToPreviousPage() {
  ...
  render() {
    ...
    return this.navElement(index.h(index.Fragment, null, this.goToPreviousPage(), this.goToFirstPage(), this.showEllipsis('front'), this.pagesFull.length >= 3 ? this.centerPages() : null, this.showEllipsis('back'), this.goToLastPage(), this.goToNextPage()));
  }
...
exports.db_pagination = DbPagination;

//# sourceMappingURL=db-pagination.cjs.entry.js.map